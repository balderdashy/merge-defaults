...
describe('Test that mergeDefaults is backwards compatible with defaults \n', function () {
  ...
  it('should assign own source properties', function() {
    ...
    Foo.prototype.b = 2;
    deepEqual(defaults({ 'c': 2 }, new Foo), { 'a': 1, 'c': 2 });
  });

  it('should accept multiple source objects', function() {
    var expected = { 'a': 1, 'b': 2, 'c': 3 };
    deepEqual(defaults({ 'a': 1, 'b': 2 }, { 'b': 3 }, { 'c': 3 }), expected);
    deepEqual(defaults({ 'a': 1, 'b': 2 }, { 'b': 3, 'c': 3 }, { 'c': 2 }), expected);
  });
  ...
  it('should not error on `null` or `undefined` sources (test in IE < 9)', function() {
    try {
      deepEqual(defaults({ 'a': 1 }, null, undefined, { 'a': 2, 'b': 2 }), { 'a': 1, 'b': 2 });
    } catch(e) {
      throw e;
    }
  });
